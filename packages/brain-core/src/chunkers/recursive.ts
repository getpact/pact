 ...
 *   1. Paragraphs (\n\n)
 *   2. Lines (\n)
 *   3. Sentences (. ! ? followed by space or newline; plus CJK equivalents)
 *   4. Clauses (; : , plus CJK equivalents)
...
import { CJK_CLAUSE_DELIMITERS, CJK_SENTENCE_DELIMITERS, countCJKAwareWords } from "../cjk.js";
...
// Upstream gbrain stripped per-page "takes" and "facts" fences here so
// private rows would not reach content_chunks. Pact does not use those
// content fences; this fork chunks raw text. If audience-aware redaction
// is needed, do it before calling chunkText.
function stripTakesFence(text: string): string {
  return text;
}
function stripFactsFence(text: string, _opts?: { keepVisibility?: string[] }): string {
  return text;
}
...
function applyOverlap(chunks: string[], overlapWords: number): string[] {
  ...
  for (let i = 1; i < chunks.length; i++) {
    ...
    const prevTrailing = extractTrailingContext(prev, overlapWords);
    result.push(prevTrailing + curr);
  }
